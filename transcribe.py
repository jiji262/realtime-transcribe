...
class HUD(QMainWindow):
  ...
  def updateTextWidget(self):
    ...
    vertical_scrollbar.setValue(vertical_scrollbar.maximum())
        ...
        # 调试信息
        print(f"HUD text updated, length: {len(current_text)}")
...
class PyAudioProvider(AudioInputProvider):
  ...
  def _record_audio(self):
    def stream_callback(in_data, frame_count, time_info, status):
        ...
        if data_size > 0:
            print(".", end="", flush=True)
            if data_size > 100:  # 确保音频数据至少包含一些内容
        self.data_queue.put(in_data)
                if self.data_queue.qsize() % 10 == 0:  # 每10个包打印一次
                    print(f"\nReceived audio data: {data_size} bytes, queue size: {self.data_queue.qsize()}")
        else:
    ...
    # Open audio stream with the selected input device
      if self.device_index is None:
        ...
        print("Using default input device to open audio stream")
        ...
        print(f"Using device index {self.device_index} to open audio stream")
    stream = self.audio.open(
      format=self.audio_format,
      channels=self.audio_channels,
      rate=self.sample_rate,
    ...
    stream.close()
      ...
      import traceback
      traceback.print_exc()
      # Try to recover with default device if specified device failed
      if self.device_index is not None:
        print("Attempting to use default device recovery...")
        self.device_index = None
        self._record_audio()  # Retry with default device
...
class Transcriber():
    ...
    if device_index is None:
      ...
      if args.input is None:
        # Choose the index of the desired input device
          # Default to first microphone if no user input in 10 seconds
          default_device = 0
          # Find first device with "mic" in the name
          for idx, name in enumerate(devices):
            if "mic" in name.lower() or "microphone" in name:
              default_device = idx
              break
          
          print(f"Default device will be {default_device}: {devices[default_device]} in 10 seconds...")
          time.sleep(10)
  ...
  def start_transribe_thread(self):
    ...
    self.transribe_thread.daemon = True
  ...
  def stop_transribe_thread(self):
    ...
    self.stop_event.set()
    self.transribe_thread.join(timeout=5.0)  # Wait up to 5 seconds
  ...
  def update_hud_text(self, text):
    ...
    with text_lock:
      ...
      if cleaned_text.strip():
        ...
        print(f"Updated text: {cleaned_text[:30]}..." if len(cleaned_text) > 30 else f"Updated text: {cleaned_text}")
      else:
  ...
  def listen(self):
    ...
    while not self.stop_event.is_set():
      try:
          ...
          try:
            # 尝试不加锁先查看队列大小
            approx_queue_size = self.data_queue.qsize()
            
            if approx_queue_size > 0:
              print(f"Detected {approx_queue_size} audio data packets")
        ...
        if args.no_faster_whisper:
          ...
          # 在实时模式下，减小所需的最小音频数据量
          min_audio_length = 0.3 if realtime_mode else 0.5  # 秒
          
          # 如果音频数据太少，就不处理，除非已经过去太长时间
          if len(acc_audio_data) < self.sample_rate * min_audio_length:
            if current_time - last_transcription_time < 2.0:
              print(f"Audio data too short ({len(acc_audio_data)/self.sample_rate:.2f} seconds), waiting for more data...")
              continue
            else:
              print(f"Although audio data is less, it has been {current_time - last_transcription_time:.2f} seconds since last transcription, attempting to process")
          ...
          for seg in segments:
            result['segments'].append({
              ...
              if not result['segments']:
                print("Warning: No text segments in transcription result")
        ...
        texts = list(map(lambda x: x['text'], result['segments']))
            ...
            if texts:
              ...
              print("Warning: No transcription text, possibly no speech in audio")
          except Exception as e:
          ...
          if texts:
            ...
            if current_result.strip():
              ...
              print("Warning: Transcription result empty, skipping update")
